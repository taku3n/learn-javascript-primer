...
- 関数: ある一連の手続き（文の集まり）を 1 つの処理としてまとめる機能。

### 関数宣言

- 関数宣言: `function` からはじまる文のこと。
  - 関数を定義している。
  - 多分 python の`def`ブロックのようなもの?
...
- 関数を構成する 4 要素
  ...
  - 関数の返り値
    ...
    - 関数の中身の `return` 文で関数の実行結果として任意の値を返すことができる。
- 関数で `return` 文が実行されると、関数内ではそれ以降の処理は行われない。
- 返り値がない場合もある。
  - この場合は関数の中身の`return`文は省略される。
  - 返り値は`undefined`となる。

### 関数の引数

- 関数に定義した仮引数の個数と実際に呼び出したときの引数の個数が違っても、関数を呼び出せる。

#### 呼び出し時の引数が少ないとき

- 余った仮引数には`undefined`が代入される。
- 例えば、2 つの引数を取る関数に 1 つしか引数を渡さないと 2 つ目の引数は`undefined`になる。
...
- 可変長引数: 固定した数ではなく任意の個数の引数を受け取れること。
  - 例えば、`Math.max(...args)`は引数を何個でも受け取り、受け取った引数の中で最大の数値を返す関数。
  - `Rest parameters`か関数の中でのみ参照できる`arguments`という特殊な変数を使う。

#### Rest parameters

- Rest parameters: 仮引数名の前に...をつけた仮引数のこと。
  - 別名は残余引数。
  - 関数に渡された値が配列として代入される。
...
- 通常の仮引数と組み合わせても定義できる。
  - 末尾の引数を`Rest parameters`になるようにする。
- Spread 構文: 配列の前に...をつけた構文のこと。
  - 配列の値を展開したものを関数の引数として渡すことができる。

#### arguments

- 関数の中でのみ参照できる特殊な変数(非推奨)。
  - 関数に渡された引数の値がすべて入った Array-like なオブジェクト。
  - インデックスで要素へアクセスできる。
  - `Array`ではないため、実際の配列とは異なり`Array`のメソッドは利用できない。
- Rest parameters が利用できる環境では、arguments 変数を使うべきではない。
  - Arrow Function では利用できないため。
  - Array のメソッドを利用できないため。
  - 仮引数だけを見て可変長引数を取ることが分かりにくいため。

### 関数の引数と分割代入

- 分割代入

  - オブジェクトや配列からプロパティを取り出し、変数として定義し直す構文のこと。
  - ES2015 で導入された。
...
- 関数は関数オブジェクトとも呼ばれ、オブジェクトの一種。
  - 関数はただのオブジェクトとは異なり、関数名に()をつけることで、関数としてまとめた処理を呼び出せる。
  - `()`をつけて呼び出されなければ、関数をオブジェクトとして参照できる。
  - 関数は変数へ代入したり、関数の引数として渡すことが可能
- ファーストクラスファンクション（第一級関数）: 関数が値として扱えること。2 つの方法がある。
  - 関数宣言と同じ function キーワードを使って値として定義する方法。
  - Arrow Function を使う方法。

#### 関数式

- 関数式: 関数を値として変数へ代入している式のこと。
  - 関数宣言は文なのに対し、関数式では関数を値として扱う。
  - 文字列や数値などの変数宣言と同じ定義方法。
  - `function`キーワードの右辺に書く関数名は省略できる。
...
- `function`キーワードの右辺に書く関数名は省略可能。
  - 名前を持たない関数を**匿名関数**（または無名関数）と呼ぶ。
...
- ES2015 以降使える機能。
- `=>`（イコールと大なり記号）を使い、匿名関数を定義する構文。
  - function キーワードに比べて短い。
  - できること/できないことがはっきりしていて人による解釈や実装の違いを起こしにくい。
  - `this`の問題の多くを解決することができる。
...
const fnC = (x) => {
  /* 仮引数が1つのみのときは()を省略可能(フォーマッタにより()が付けられている) */
};
...
- 配列の map メソッドを使用した比較
- function キーワードの関数式では、値を返すコールバック関数を次のように書く。
...
- 同じ名前の関数を複数回宣言した場合には、後ろで宣言された関数によって上書きされる。
  - 同じ関数名で複数の関数を定義することは基本的に意味がないため避けるべき。
...
- 関数定義の上書きが起こる場合
  - function キーワードでの関数宣言
  - var キーワードを使った関数式
- 関数定義の上書きが起こらずエラーになる場合
  - let や const

### コールバック関数

- コールバック関数
  - その場で作った匿名関数を関数の引数（値）として渡す場合、引数として渡される関数のこと
  - 非同期処理においてもよく利用される
- 高階関数
  - コールバック関数を引数として使う関数やメソッドのこと
...
- 例
  - 配列の forEach メソッドはコールバック関数を引数として受け取る高階関数
  - forEach メソッドは、配列の各要素に対してコールバック関数を一度ずつ呼び出す
...
- コールバック関数となる匿名関数をその場で定義して渡すとこう短く書ける。
...
- メソッド: オブジェクトのプロパティである関数
  - 関数とメソッドの機能的な違いはない。
  - `オブジェクト.メソッド名()`と書くことで呼び出せる。
- 例
  - obj の method1 プロパティと method2 プロパティに関数を定義
...
- 例
  - 空オブジェクトの obj を定義してから、method プロパティへ関数を代入してもメソッドを定義できる
...
- オブジェクトリテラルの中で `メソッド名(){ /*メソッドの処理*/ }` と書ける。
  - オブジェクトのメソッドだけではなく、クラスのメソッドと共通の書き方
  - この短縮記法に統一するのが better
